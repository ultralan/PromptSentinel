...
src/report/     只读 runs 产物，生成训练曲线等可视化报告
```
...
uv run python -m src.report.main --run-dir runs/<训练运行目录>
```
...
训练结束后，报告模块可只读 `trainer_state.json`，在对应运行目录下写入 `plots/loss_curve.png` 和 `plots/loss_curve.json`；生成报告不修改模型、checkpoint 或训练状态。
